...
parser/
├─ rua_parser.js            Aggregate report XML parser (RFC 7489) with validation
└─ ruf_parser.js            Forensic report parser (RFC 6591 / AFRF) with validation
dashboard/
├─ dashboard.html           Main dashboard UI
├─ dashboard.css            Styles with CSS variables & dark mode
└─ dashboard.js             Dashboard logic — scan, parse, render, issues display
options/
...
→ XML Parse → Normalize → Validate → Deduplicate → Aggregate → Dashboard Render
                                ↓
